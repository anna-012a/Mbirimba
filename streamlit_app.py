...
# We use st.cache_resource so it doesn't try to reconnect every single click
@st.cache_resource
def get_connection():
...
try:
    # Force read using the service account in secrets
    df = conn.read(worksheet="Sheet1", ttl=0)
    series_dad = int(df.iloc[0]["Dad"])
    series_mom = int(df.iloc[0]["Mom"])
except Exception as e:
    st.error(f"Waiting for database connection... Error: {e}")
...
if dad_total >= WINNING_SCORE or mom_total >= WINNING_SCORE:
    ...
    if st.button(f"Save Win for {winner}", use_container_width=True):
        new_dad = series_dad + 1 if winner == "Dad" else series_dad
        new_mom = series_mom + 1 if winner == "Mom" else series_mom
        conn.update(worksheet="Sheet1", data=pd.DataFrame([{"Dad": new_dad, "Mom": new_mom}]))
...
if st.session_state.first_dealer is None:
    ...
    with st.form("score_form", clear_on_submit=True):
        ...
        d_in = f1.number_input("Dad", step=5, value=0)
        m_in = f2.number_input("Mom", step=5, value=0)
        if f3.form_submit_button("OK", use_container_width=True):
            st.session_state.scores["Dad"] += d_in
            st.session_state.scores["Mom"] += m_in
            st.session_state.history.append({"Rd": round_num, "Dad": st.session_state.scores["Dad"], "Mom": st.session_state.scores["Mom"]})
            st.rerun()
